---
description: 'Generate single-file interactive HTML pages for learning any user-provided topic, with scroll animations, syntax highlighting, and quizzes - Brought to you by microsoft/hve-core.'
...
The default stack SHOULD remain unchanged unless the user explicitly requests alternatives.
...
* The topic MUST be confirmed before proceeding to content architecture.
...
The user MUST explicitly confirm the content plan before proceeding to Phase 3.
...
7. The page MUST include at least one interactive element (quiz, exercise, or knowledge check).
...
All interactive features MUST use native HTML and vanilla JS:
...
* `graph` MAY be used as an alternative to `flowchart` when simpler syntax suffices.
* Avoid `block-beta` diagrams. They render with overlapping text at common viewports and do not support reliable label sizing.
* Avoid `classDiagram` for non-UML content. Prefer `flowchart` with subgraphs for architecture layers.
* Node labels SHOULD remain concise (under 60 characters). Use `<br>` for line breaks inside labels. Do not use `\n`, as it renders as literal text when Mermaid runs inside HTML.
...
Every generated HTML file MUST apply these patterns:
...
A generated page meets quality standards when:

* The HTML file opens in a browser without errors and renders all sections.
* Scroll animations trigger on each major section during scrolling.
* Syntax highlighting applies to all code blocks.
* Mermaid diagrams render without errors (when included).
* The interactive quiz or knowledge check scores answers correctly.
* The page supports both light and dark color schemes.
* Content progresses logically from foundational concepts to advanced topics.
* All CDN resources load successfully with no console errors.
